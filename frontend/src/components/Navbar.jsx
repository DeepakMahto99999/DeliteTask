...
const Navbar = () => {
    ...
    return (
        <header className='flex items-center justify-between shadow text-sm py-4 border-b border-gray-400 px-4'>
            {/* Logo */}
            ...
            <nav className='hidden md:flex items-center gap-5 font-medium'>
                <NavLink to='/' className='relative group'>
                    ...
                    <hr className='absolute bottom-0 left-1/2 transform -translate-x-1/2 border-none h-0.5 bg-[#5f6FFF] w-3/5 scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-center' />
                </NavLink>
                <NavLink to='/dashboard' className='relative group'>
                    <li className='py-1 list-none'>Dashboard</li>
                    <hr className='absolute bottom-0 left-1/2 transform -translate-x-1/2 border-none h-0.5 bg-[#5f6FFF] w-3/5 scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-center' />
                </NavLink>
            </nav>
            ...
            {/* Mobile Menu */}
            <div
                className={`fixed top-0 right-0 bottom-0 z-20 bg-white transition-all duration-300 overflow-hidden md:hidden ${showMenu ? 'w-full' : 'w-0 h-0'
                    }`}
            >
                ...
                <ul className='flex flex-col items-center gap-4 mt-5 px-5 text-lg font-medium'>
                    <NavLink
                        onClick={() => setShowMenu(false)}
                        to='/'
                        className='px-4 py-2 rounded hover:bg-gray-100 w-full text-center'
                    >
                        Home
                    </NavLink>
                    <NavLink
